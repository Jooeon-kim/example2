...
import { setOptionNoncoffee } from "./mainSlice";

function OptionNoncoffee() {
  const { id } = useParams();
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const item = useSelector((state) =>
    state.main.nonCoffeeList.find((i) => i.id === Number(id))
  );
    ...
    <div className="wrapper option_wrap">
      ...
      >
        ...
        <div className="amount">
          ...
          <div className="right">
            <button type="button" className="minus" onClick={handleMinus}>
              -
            </button>
